...
Beware that Pydantic models have some differences with regular hinting, see the [docs](https://dlthub.com/docs/general-usage/schema-contracts#use-pydantic-models-for-data-validation)
...
Run the base case (See example 8 for a version using Pydantic models)

```bash
python dlt_tutorial/7_sample_pipeline_data_contracts.py --refresh
```

Modify the value of "created_at" to any random string that is not a timestamp, e.g. "not_a_timestamp"

Run the code again. What happened? How would you prevent this? Implement the necessary changes.
...
Feel free to reach out with any questions or suggestions at my email <mailto:daquintanav@gmail.com>
